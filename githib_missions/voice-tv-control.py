#!/home/poligon/.local/bin/checkio --domain=py run voice-tv-control
...
class VoiceCommand:
    def __init__(self, chanels):
        self.chanels = chanels.copy()
        self.chanel = 0

    def current_channel(self):
        return self.chanels[self.chanel]
    ...
    def last_channel(self):
        self.chanel = len(self.chanels) - 1
        return self.current_channel()

    def turn_channel(self, number):
        self.chanel = number - 1
        return self.current_channel()

    def next_channel(self):
        self.chanel = (self.chanel + 1) % len(self.chanels)
        return self.current_channel()

    def previous_channel(self):
        self.chanel = self.chanel - 1 if self.chanel != 0 else \
            len(self.chanels) - 1
        return self.current_channel()

    def is_exist(self, channel):

        if isinstance(channel, type(str())):
            if channel in self.chanels:
                return "Yes"
        else:
            if channel > -1 and ((channel + 1) <= len(self.chanels)):
                return "Yes"
        return "No"
...
# previous_channel()- turns on the previous channel. If the current channel is the first one, turns on the last channel.
# current_channel()- returns the name of the current channel.
# is_exist(N/'name')- gets 1 argument - the number N or the str
