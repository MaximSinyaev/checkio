#!/home/poligon/.local/bin/checkio --domain=py run the-square-chest
...
def checkio(lines_list: List[List[int]]) -> int:
    ...
    matrix = [[0 for i in range(3)] for i in range(5)]
    for x_y in lines_list:
        matrix[(x_y[0] // 4)][(x_y[0] % 4) - 1] = 1
        matrix[(x_y[1] // 4)][(x_y[1] % 4) - 1] = 1
    for i in matrix:
        print(i)
    return 0
