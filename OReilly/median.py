#!/home/poligon/.local/bin/checkio --domain=py run median
...
def search_median(data, mid):
    while len(data) > 0:
        median = random.choice(data)
        left = [i for i in data if i < median]
        right = [i for i in data if i > median]
        if len(left) > mid:
            data = left
        elif len(left) + (len(data) - len(left) - len(right)) > mid:
            return median
        else:
            mid = mid - len(left) - (len(data) - len(left) - len(right))
            data = right

def checkio(data: List[int]) -> [int, float]:
    if len(data) % 2 == 0:
        return 0.5 * (search_median(data, (len(data)) // 2) +
                      search_median(data, (len(data)) // 2 - 1))
    else:
        return search_median(data, (len(data)) // 2)
